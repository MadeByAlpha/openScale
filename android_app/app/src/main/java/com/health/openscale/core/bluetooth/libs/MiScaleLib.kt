...
class MiScaleLib(
    ...
    override fun getMuscle(weight: Float, impedance: Float): Float {
        ...
        val smmKg: Float = if (impedance > 0f) {
            ...
            0.401f * h2OverR + 3.825f * (if (isMale) 1f else 0f) - 0.071f * age + 5.102f
        } else {
            // Fallback: approximate as fraction of LBM
            val lbm = getLBM(weight, impedance)
            val ratio = if (isMale) 0.52f else 0.46f
            lbm * ratio
        }
    ...
    override fun getBodyFat(weight: Float, impedance: Float): Float {
        ...
        if (isMale && weight < 61.0f) {
            coeff = 0.98f
        } else if (!isMale && weight > 60.0f) {
            coeff = 0.96f
            if (height > 160.0f) {
                coeff *= 1.03f
            }
        } else if (!isMale && weight < 50.0f) {
            coeff = 1.02f
            if (height > 160.0f) {
                coeff *= 1.03f
            }
