...
class OneByoneLibTest {
    ...
    private val FIXTURES = mapOf(
        ...
        "male_high" to Snap(
            isMale = true, age = 52, h = 175f, w = 95f, imp = 430f, pt = 2,
    ...
    fun snapshots_match_expected_outputs() {
        ...
        FIXTURES.forEach { (name, s) ->
            val lib = OneByoneLib(s.isMale, s.age, s.h, s.pt)

            assertWithMessage("$name:bmi").that(lib.getBMI(s.w)).isWithin(EPS).of(s.bmi)
            assertWithMessage("$name:bf").that(lib.getBodyFat(s.w, s.imp)).isWithin(EPS).of(s.bf)
            assertWithMessage("$name:lbm").that(lib.getLBM(s.w, s.imp)).isWithin(EPS).of(s.lbm)
            assertWithMessage("$name:muscle").that(lib.getMuscle(s.w, s.imp)).isWithin(EPS).of(s.muscle)
            assertWithMessage("$name:water").that(lib.getWater(s.w, s.imp)).isWithin(EPS).of(s.water)
            assertWithMessage("$name:bone").that(lib.getBoneMass(s.w, s.imp)).isWithin(EPS).of(s.bone)
            assertWithMessage("$name:vf").that(lib.getVisceralFat(s.w, s.imp)).isWithin(EPS).of(s.vf)
        }
    }
    ...
    fun bmi_monotonicity_weightUp_increases_heightConstant() {
        val lib = OneByoneLib(true, 30, 180f, 0)
        val w1 = 70f
        val w2 = 85f
    ...
    fun bmi_monotonicity_heightUp_decreases_weightConstant() {
        val libShort = OneByoneLib(true, 30, 170f, 0)
        val libTall  = OneByoneLib(true, 30, 190f, 0)
        val w = 80f
    ...
    fun water_switch_coeff_below_and_above_50() {
        val lib = OneByoneLib(false, 40, 165f, 1)
    ...
    fun boneMass_isReasonablyClamped_between_0_5_and_8_0() {
        val lib = OneByoneLib(false, 55, 170f, 2)
    ...
    fun muscle_reacts_to_impedance_reasonably() {
        val lib = OneByoneLib(true, 30, 180.0f, 0)
    ...
    fun bodyFat_stays_within_reasonable_bounds() {
        val lib = OneByoneLib(true, 35, 180f, 1)
    ...
    fun peopleType_influences_outputs() {
        val base = OneByoneLib(true, 40, 175f, 0)
        val mid  = OneByoneLib(true, 40, 175f, 1)
        val high = OneByoneLib(true, 40, 175f, 2)
    ...
    fun sex_flag_affects_outputs() {
        val male   = OneByoneLib(true, 32, 178f, 1)
        val female = OneByoneLib(false, 32, 178f, 1)
    ...
    fun print_current_outputs_for_fixtures() {
        fun dump(isMale: Boolean, age: Int, h: Float, w: Float, imp: Float, pt: Int) {
            val lib = OneByoneLib(isMale, age, h, pt)
            println(
                "SNAP -> isMale=$isMale age=$age h=$h w=$w imp=$imp pt=$pt | " +
                        "bmi=${lib.getBMI(w)}; bf=${lib.getBodyFat(w, imp)}; " +
                        "lbm=${lib.getLBM(w, imp)}; muscle=${lib.getMuscle(w, imp)}; " +
                        "water=${lib.getWater(w, imp)}; bone=${lib.getBoneMass(w, imp)}; vf=${lib.getVisceralFat(w, imp)}"
            )
        }

        // Re-run if you intentionally modify formulas; then paste outputs into FIXTURES above.
        dump(true, 30, 180f, 80f, 500f, 0)
        dump(false, 28, 165f, 60f, 520f, 1)
        dump(true, 52, 175f, 95f, 430f, 2)
        dump(true, 25, 178f, 72f, 80f, 0)
        dump(false, 35, 170f, 68f, 300f, 2)
        dump(true, 45, 182f, 90f, 1300f, 1)
    }
}
